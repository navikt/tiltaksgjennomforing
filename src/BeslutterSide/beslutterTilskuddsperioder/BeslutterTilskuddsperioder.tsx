...
const BeslutterTilskuddsPerioder: FunctionComponent<Props> = props => {
    const { avtale, godkjennTilskudd, avslåTilskudd } = useContext<Context>(AvtaleContext);
    const { enhet, setEnhetFeil, setVisAvslag, visAvslag } = useContext<Periode>(TilskuddsperiodeContext);
    ...
    const hentAvslagsArsaker = (periode: TilskuddsPeriode): string =>
        new Array(Object.values(periode.avslagsårsaker))
            .map(e => e).join(", ").split(",")
            .map(e => tilskuddsperiodeAvslagTekst[e.trim() as Avslagsårsaker]).join(", ")
                    ...
                    {avtale.tilskuddPeriode.map((periode, index) => {
                        ...
                        return (
                            <React.Fragment key={index}>
                                <tr
                                    key={index}
                                    className={cls.element('tilskuddsperiode-rad', settStylingForTabellrad(periode))}
                                >
